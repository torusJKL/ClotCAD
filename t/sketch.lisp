    ...
    (let ((old-edge (symbol-function 'cl-occt:make-edge))
      ...
      (unwind-protect
           (let ((result (rect (pnt 0 0) 10 20)))
             ...
             (assert-equal 1 (length wire-calls)
    ...
    (let ((old-edge (symbol-function 'cl-occt:make-edge))
      ...
      (unwind-protect
           (progn
             ...
             (assert-equal '(1.0d0 22.0d0 1.0d0 2.0d0) (first edges))
             (assert-equal '(11.0d0 22.0d0 1.0d0 22.0d0) (second edges))
             (assert-equal '(11.0d0 2.0d0 11.0d0 22.0d0) (third edges))
             (assert-equal '(1.0d0 2.0d0 11.0d0 2.0d0) (fourth edges)))
        (setf (symbol-function 'cl-occt:make-edge) old-edge)
        (setf (symbol-function 'cl-occt:make-wire) old-wire)))))
    ...
    (let ((old-circle (symbol-function 'cl-occt:make-circle-edge))
      ...
      (unwind-protect
           (let ((result (circle (pnt 0 0) 5)))
             ...
             (assert-equal '(0.0d0 0.0d0 5.0d0) (cdar circle-calls))
             (assert-eql :wire (first result)))
    ...
    (let ((old-edge (symbol-function 'cl-occt:make-edge))
      ...
      (unwind-protect
           (progn
             ...
             (assert-equal '(5.0d0 0.0d0 2.5d0 5.0d0) (second edges)
                            "edge2 should connect B→C")
             (assert-equal '(0.0d0 0.0d0 5.0d0 0.0d0) (third edges)
                            "edge3 should connect A→B"))
        (setf (symbol-function 'cl-occt:make-edge) old-edge)
        (setf (symbol-function 'cl-occt:make-wire) old-wire)))))
      ...
      (let ((old-edge (symbol-function 'cl-occt:make-edge))
        ...
        (unwind-protect
             (progn
               ...
               (assert-equal '(2.0d0 23.0d0 2.0d0 3.0d0) projected-point))
          (setf (symbol-function 'resolve-shape) old-resolve)
      ...
      (unwind-protect
           (let ((result (extrude-from-face :box/top-face :mock-sketch
             ...
             (destructuring-bind (prism-call cut-call) (reverse calls)
               ...
               (assert-equal 5.0d0 (abs (fifth prism-call))
  ...
  (let ((pkg (find-package :clotcad-user)))
    (dolist (sym '(pnt sketch-on-face rect circle slot polygon line-chain
      ...
      (let ((found (find-symbol (string sym) pkg)))
        ...
        (assert-eql :inherited (nth-value 1 (find-symbol (string sym) pkg))
                    (format nil "~S should be inherited (via :use) in :clotcad-user" sym))))))
