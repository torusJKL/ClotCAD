...
### Requirement: Category tree display on no-argument apropos
...
#### Scenario: No-argument apropos shows category tree
- **WHEN** the user types `(apropos)` in the REPL
...
- **WHEN** the user calls `(apropos)` after a new function is defined in a cl-occt source file
...
- **WHEN** the user types `(apropos)` in the embedded GUI REPL (where `*standard-output*` is a string stream)
...
- **WHEN** the user types `(apropos)` via a Slynk or Alive LSP connection
...
- **WHEN** the user types `(apropos :fillet)`
...
- **WHEN** the user types `(apropos :file)`
...
- **WHEN** the user types `(apropos :boogers)`
...
- **WHEN** the user types `(apropos :face)`
...
- **WHEN** the user types `(apropos "box")`
...
- **THEN** `(apropos)` SHALL show a category "Widget" for functions from that file

### Requirement: Documentation updated

The `docs/clotcad-api.md` file SHALL be updated to document the new `apropos` modes.

#### Scenario: Introspection section documents new apropos modes
- **WHEN** the user reads the documentation for `apropos`
