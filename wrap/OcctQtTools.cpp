...
QSurfaceFormat OcctQtTools::qtGlSurfaceFormat()
{
                                     ...
                                     const QMouseEvent* theEvent)
{
  ...
  const NCollection_Vec2<double> aPnt2d(theEvent->position().x(), theEvent->position().y());
  const NCollection_Vec2<int> aPnt2i(theView->Window()->ConvertPointToBacking(aPnt2d) + NCollection_Vec2<double>(0.5));
...
Aspect_VKey OcctQtTools::qtKey2VKey(int theKey)
{
  switch (theKey)
  {
    ...
    case Qt::Key_Return: return Aspect_VKey_Enter;
